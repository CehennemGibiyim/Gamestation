...
Tkinter GUI | Python 3.x | 18 Tema + Özel Başlık Çubuğu

Albion Online Vergi Sistemi:
  - Alış:  market_fee (varsayılan %2.5)
  - Satış: market_fee (varsayılan %4.0) + setup_fee (varsayılan %0.5)
  - Net Kazanç = OyunFiyat×adet×(1-satış%) - AlışFiyat×adet×(1+alış%)

Düzeltmeler v2.1:
  - Alt+Tab / Görev çubuğu sorunu giderildi
  - Net fiyat artık otomatik hesaplanıyor (elle giriş kaldırıldı)
  - Albion Online'a uygun varsayılan oranlar ve Silver (S) birimi
"""
...
DEFAULT_ITEMS = [
    # (kategori, tier, oyun_ici_fiyat, alis_fiyati, adet)
    ("Lux Mallar",           "T4",    1_000,      900,   0),
    ("Lux Mallar",           "T5",    5_000,    4_500,   0),
    ("Lux Mallar",           "T6",   25_000,   23_000,   0),
    ("Kraliyet Oyuncaklari", "T6",   10_000,    9_000,   0),
    ("Kraliyet Oyuncaklari", "T7",   50_000,   45_000,   0),
    ("Kraliyet Oyuncaklari", "T8",  250_000,  230_000,   0),
]
...
class LuxApp(tk.Tk):
    def __init__(self):
        ...
        # ── Alt+Tab Düzeltmesi ────────────────────────────────────────────────
        # overrideredirect(True) doğrudan ana pencereye uygulanırsa pencere
        # görev çubuğunda görünmez ve Alt+Tab çalışmaz.
        # Çözüm: Ana pencere tamamen şeffaf/ekran dışı "hayalet" kalır;
        #        asıl UI child Toplevel üzerinde açılır.
        #        Görev çubuğu ikonu ana pencereye ait olduğundan Alt+Tab çalışır.
        self.title("Lux Mallar Kar Hesaplama")
        self.geometry("1x1+-10000+-10000")
        self.attributes("-alpha", 0.0)
        self.deiconify()  # görev çubuğunda göster

        self._win = tk.Toplevel(self)
        self._win.overrideredirect(True)
        self._win.attributes("-alpha", 0.96)
        self._win.geometry("1300x860")
        self._win.protocol("WM_DELETE_WINDOW", self._on_close)

        # Alt+Tab ile ana pencereye odaklanıldığında _win'i öne çıkar
        self.bind("<FocusIn>", lambda e: self._win.lift())
        # ─────────────────────────────────────────────────────────────────────
    ...
    def _build_items(self):
        self.items = []
        for (kat, tier, oyun_fiyat, alis, adet) in DEFAULT_ITEMS:
            self.items.append({
                "kat": kat, "tier": tier,
    ...
    def _build_titlebar(self, parent):
        ...
        win_btn("#F85149", self._on_close,       "#FF7B72")
        ...
        tk.Label(rf, text="v2.1  Python GUI",
    ...
    def _dot_enter(self, canvas, th, idx):
        ...
        self._tooltip_win = tk.Toplevel(self._win)
    ...
    def _minimize(self):
        self._win.withdraw()
        self.iconify()
        self.bind("<Map>", self._restore_from_minimize)

    def _restore_from_minimize(self, e):
        self.unbind("<Map>")
        self.attributes("-alpha", 0.0)
        self._win.deiconify()
        self._win.lift()
    ...
    def _build_settings(self, parent):
        ...
        self._sv = {}
        sec("ALIS VERGILERI")
        for key, lbl_txt in [("alis_vergi",    "Market Alis Ucreti"),
                              ("alis_komisyon", "Ek Komisyon")]:
        ...
        sec("SATIS VERGILERI")
        for key, lbl_txt in [("satis_vergi",    "Market Satis Ucreti"),
                              ("satis_komisyon", "Setup Fee / Ek"),
                              ("kdv",            "Diger Kesinti")]:
            v = tk.StringVar(value=str(self.settings[key]))
            row_field(lbl_txt, v)
            v.trace_add("write", lambda *a, k=key, vv=v: self._sv_changed(k, vv))
        ...
        tk.Label(parent, text=note, font=("Consolas", 7),
                 bg=T["bg2"], fg=T["text2"],
    ...
    def _build_rows(self):
        ...
        for idx, item in enumerate(self.items):
            ...
            tk.Frame(hdr_band, bg=T["gold"], height=1).pack(fill="x", padx=8)
            ...
            # Sadece oyun içi fiyat, alış fiyatı ve adet — net_fiyat otomatik
            for key, var, w_s in [("oyun_fiyat", item["oyun_fiyat"], 13),
                                   ("alis",       item["alis"],       13),
                                   ("adet",       item["adet"],        9)]:
    ...
    # ── Hesaplama (Albion Online Mantığı) ────────────────────────────────────
    def _get_rates(self):
        def pct(k):
            try:
        ...
        r_buy  = pct("alis_vergi")  + pct("alis_komisyon")
        r_sell = pct("satis_vergi") + pct("satis_komisyon") + pct("kdv")
        return r_buy, r_sell
    ...
    def _recalc(self):
        ...
        for idx, item in enumerate(self.items):
            ...
            # ── Albion Online Vergi Hesabı ───────────────────────────────
            # Alış: aldığın fiyat + market fee (fazladan ödersin)
            maliyet   = alis * adet * (1 + r_buy)

            # Satış: oyun içi fiyattan market fee düşülür (elinde kalan)
            net_satis = oyun_fiyat * adet * (1 - r_sell)
            ...
            ww = self.row_widgets[idx]
            ww["maliyet"].config(text=fmt(maliyet) + " S")
            ww["net_satis"].config(text=fmt(net_satis) + " S")
            kc = T["accent2"] if kazanc >= 0 else T["danger"]
            ww["kazanc"].config(text=fmt(kazanc) + " S", fg=kc)
            ...
            lines = [
                (f"Alis Ham Toplam",                  f"{fmt(alis_ham)} S",        T["text"]),
                (f"  + Alis Fee (%{r_buy*100:.2f})",  f"+ {fmt(alis_vergi_tl)} S", T["danger"]),
                (f"  = Toplam Alis Maliyeti",          f"  {fmt(maliyet)} S",       T["text"]),
                ("",                                   "",                           T["text"]),
                (f"Oyun Ici Fiyat Toplam",             f"{fmt(satis_ham)} S",       T["text"]),
                (f"  - Satis Fee (%{r_sell*100:.2f})", f"- {fmt(satis_vergi_tl)} S",T["danger"]),
                (f"  = Net Satis (Eline Gecen)",       f"  {fmt(net_satis)} S",     T["text"]),
                ("",                                   "",                           T["text"]),
                ("NET KAZANC",                          f"{fmt(kazanc)} S",          kc),
            ]
            for dl, dv, dc in lines:
                ...
                tk.Label(dr, text=dl, font=("Consolas", 8),
                         bg=T["sep"], fg=T["text2"],
                         width=36, anchor="w").pack(side="left", padx=(22, 0))
        ...
        self.lbl_sum_maliyet.config(text=fmt(totals["maliyet"])   + " S")
        self.lbl_sum_net_satis.config(text=fmt(totals["net_satis"]) + " S")
        tc = T["gold"] if totals["kazanc"] >= 0 else T["danger"]
        self.lbl_sum_kazanc.config(text=fmt(totals["kazanc"]) + " S", fg=tc)
        for kat, lbl in self.lbl_kat.items():
            v = kat_k.get(kat, 0)
            lbl.config(text=fmt(v) + " S",
